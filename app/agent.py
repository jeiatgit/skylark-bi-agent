...
# Robust list of models on Google AI Studio
MODELS = ["gemini-flash-lite-latest", "gemini-3.6-flash", "gemini-flash-latest", "gemma-4-31b-it"]
...
def call_gemini(prompt: str, system_instruction: str = "") -> str:
    ...
    for model in MODELS:
        ...
        try:
            ...
            if "error" in data:
                last_err = data["error"]
                print(f"[Gemini Fallback] Model {model} error: {data['error'].get('message', '')}. Trying next model...")
                continue
            ...
            last_err = e
            print(f"[Gemini Fallback] Network error on {model}: {e}. Trying next model...")
            continue

    raise RuntimeError(f"All AI models temporarily busy ({last_err}). Please retry in a few seconds.")
...
def run_agent(user_message: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[Agent Router Fallback] Using default KPI tool due to: {e}")
        tool_name = "get_kpis"
        tool_args = {}

    # Execute pandas deterministic calculations
    ...
    try:
        ...
        print(f"[Agent Synthesis Fallback] {e}")
        k = tool_results.get("summary", tool_results)
        final_text = (
            f"### Business Intelligence Summary\n\n"
            f"Based on our live monday.com dataset, here are the computed metrics:\n\n"
